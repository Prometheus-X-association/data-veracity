...
import { normaliseError } from '../src/api/templates.js'
import { normalizeEvaluationResult, failureFromCode } from '../src/failures/failureModel.js'

test('keeps the VLA failure model inside its Docker build context', () => {
  const source = fs.readFileSync(new URL('../src/failures/failureModel.js', import.meta.url), 'utf8')
