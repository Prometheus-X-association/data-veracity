const DEFINITIONS = {
  ...
  VERIFICATION_REJECTED: { status: 'failed', title: 'Attestation was rejected', summary: 'The verifier could not accept the attestation claims.', nextAction: 'Review the verification evidence and request a valid attestation.', retryable: false },
  VERIFICATION_PENDING: { status: 'pending', title: 'Verification is still pending', summary: 'No final verification result is available.', nextAction: 'Wait for verification to finish and refresh the result.', retryable: true },
  ...
  EVALUATION_OPERATIONAL_FAULT: { status: 'error', title: 'Evaluation could not run', summary: 'The processing service could not complete the evaluation.', nextAction: 'Check the processing service and retry when it is healthy.', retryable: true },
