...
def build_vla_assistant_messages(
    ...
    system = (
        ...
        "- message: a short reply to the author, always written in English.\n"
        "- metadata: the VLA metadata, always an object with exactly the keys "
        "name (required before the VLA can be created) and description "
        "(optional), both strings. Start from the builder context's metadata "
        "and keep what the author has entered unless they ask to change it. "
        "Propose a short name and a one-sentence description of what the VLA "
        "guarantees from the request and the requirements. If the request "
        "gives you nothing to name the VLA after, leave name empty and ask "
        "the author for a name in message.\n"
        ...
        "template fits.\n"
        '- missingTemplates: an array of {"name", "reason"} objects, one for '
        "each template the author still has to create because no catalog "
        "template can express the rule. Each entry must be exactly one "
        "template the author can create on its own, so split a request that "
        "needs several. name is a short title for that template; reason "
        "describes the rule precisely enough that the template can be "
        ...
        "- requirements is the complete list of requirements the VLA should "
        "have, not only the ones you add. The builder context's fragments "
        "are the requirements already attached (data.id is the templateId, "
        "data.model the model): include each of them unchanged unless the "
        "author asks to remove or change it, because leaving one out removes "
        "it from the VLA. The same template may be used more than once with "
        "different models, but never list the same template and model "
        "twice.\n"
        ...
        "- Each reply replaces the previous draft. The builder context's "
        "draft, when present, is that previous draft. When the author says "
        "they have created templates and asks you to check again, look for "
        "them in the catalog and return the complete draft: keep the "
        "requirements that still apply, add requirements for the rules the "
        "new templates now cover, and keep in missingTemplates only the "
        "templates that are still missing.\n"
