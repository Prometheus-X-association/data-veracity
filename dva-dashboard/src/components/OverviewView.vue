<template>
  <section>
    ...
    <div class="content-grid">
      <div class="panel health-panel">
        ...
        <div class="health-list">
          <div v-for="service in services" :key="service.name" class="health-row"><div class="service-icon"><v-icon :name="service.icon" /></div><div class="service-copy"><strong>{{ service.name }}</strong><span>{{ service.description }}</span></div><div class="service-latency">{{ service.latency }}</div><div class="health-check" :class="service.status" :title="service.detail || statusLabel(service.status)"><v-icon :name="healthIcon(service.status)" /></div></div>
        </div>
    ...
    <div v-if="failureItems.length" class="panel attention-panel">
      <div class="panel-heading"><div><h2>Needs attention</h2><p>Recent records with an incomplete evaluation or verification.</p></div><router-link to="/requests">Open attestations <v-icon name="fa-arrow-right" /></router-link></div>
      <div class="attention-list"><div v-for="item in failureItems" :key="item.id" class="attention-row"><div class="attention-mark" :class="item.failure.status"><v-icon :name="item.failure.status === 'pending' ? 'fa-clock' : 'fa-exclamation-triangle'" /></div><div class="attention-copy"><strong>{{ item.failure.title }}</strong><span>{{ item.exchange }} · {{ item.failure.code || 'Review required' }}</span></div><p>{{ item.failure.nextAction }}</p></div></div>
    </div>
...
import { fetchHealthRows } from '../health/fetchHealth.js'
import { checkingRows, demoRows, overallLabel as labelFor, overallTone as toneOf, statusLabel } from '../health/healthModel.js'
...
const services = ref(checkingRows())
const overallTone = computed(() => toneOf(services.value))
const overallLabel = computed(() => labelFor(overallTone.value))
const HEALTH_ICONS = { operational: 'fa-check', degraded: 'fa-exclamation-circle', unavailable: 'fa-exclamation-triangle', checking: 'fa-clock' }
const healthIcon = status => HEALTH_ICONS[status]
...
async function checkServices () {
  if (demoMode.value) {
    services.value = demoRows()
    return
  }
  services.value = checkingRows()
  services.value = await fetchHealthRows()
}
...
<style scoped>
  ...
  .bar-area { flex: 1; } .bar-caption { color: #475569; font-size: .8rem; } .bar-caption strong { color: #0f172a; } .progress-track { height: 12px; margin: 13px 0 12px; overflow: hidden; border-radius: 99px; background: #f1f5f9; } .progress-fill { height: 100%; border-radius: inherit; background: #06b6d4; } .bar-legend { justify-content: flex-start; gap: 18px; color: #64748b; font-size: .68rem; } .bar-legend i { display: inline-block; width: 7px; height: 7px; margin-right: 5px; border-radius: 50%; } .legend-pass { background: #06b6d4; } .legend-fail { background: #cbd5e1; } .score-label { margin-left: auto; } .score-label strong { color: #0f172a; }
  .attention-panel{margin-bottom:27px}.attention-list{display:grid;gap:8px}.attention-row{display:grid;grid-template-columns:auto minmax(150px,1fr) minmax(200px,1.5fr);align-items:center;gap:10px;padding:10px 0;border-top:1px solid #f1f5f9}.attention-mark{display:grid;place-items:center;width:28px;height:28px;border-radius:8px;color:#b91c1c;background:#fee2e2;font-size:.68rem}.attention-mark.pending{color:#92400e;background:#fef3c7}.attention-copy{display:grid;gap:3px;min-width:0}.attention-copy strong{overflow:hidden;color:#334155;font-size:.76rem;text-overflow:ellipsis;white-space:nowrap}.attention-copy span{color:#94a3b8;font-size:.65rem}.attention-row p{color:#64748b;font-size:.7rem;line-height:1.4}
  @media (max-width: 1050px) { .metric-grid { grid-template-columns: repeat(2, 1fr); } .content-grid { grid-template-columns: 1fr; } } @media (max-width: 700px) { .hero-row, .performance-panel { align-items: flex-start; flex-direction: column; } .hero-actions { width: 100%; justify-content: space-between; } .metric-grid { grid-template-columns: 1fr; } .bar-legend { flex-wrap: wrap; } .score-label { margin-left: 0; } .attention-row{grid-template-columns:auto 1fr}.attention-row p{grid-column:2} }
  @media (max-width: 760px) { h1 { font-size: clamp(1.65rem, 8vw, 2rem); } .hero-actions { align-items: stretch; flex-wrap: wrap; gap: 10px; white-space: normal; } .primary-button { min-height: 44px; justify-content: center; } .performance-panel { gap: 22px; } .donut-wrap { align-self: center; } .panel { padding: 16px; } .panel-heading { align-items: flex-start; gap: 10px; } .health-row, .activity-row { align-items: flex-start; } .service-latency { margin-left: auto; } }
</style>
