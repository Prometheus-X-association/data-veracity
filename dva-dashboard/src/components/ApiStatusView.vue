<template>
  <section>
    <div class="page-header">
      <div><div class="eyebrow">INTEGRATION MONITOR</div><h2>Gateway status</h2><p>Readiness of every DVA service, as collected by the gateway.</p></div>
    ...
    <div class="status-grid"><div v-for="service in services" :key="service.name" class="status-card"><div class="status-icon"><v-icon :name="service.icon" /></div><div><strong>{{ service.name }}</strong><span>{{ service.description }}</span></div><p v-if="service.detail" class="status-detail">{{ service.detail }}</p><div class="status-bottom" :class="service.status"><span><i></i> {{ statusLabel(service.status) }}</span><b>{{ service.latency }}</b></div></div></div>
...
import { checkingRows, demoRows, overallLabel as labelFor, overallNote as noteFor, overallTone as toneOf, statusLabel } from '../health/healthModel.js'
...
const overallNote = computed(() => noteFor(overallTone.value))
...
.status-badge.warning{color:#92400e;background:#fffbeb}.status-badge.warning i{background:#f59e0b}.status-bottom.degraded{color:#92400e}.status-bottom.degraded i{background:#f59e0b}.status-detail{grid-column:1/-1;color:#64748b;font-size:.72rem;line-height:1.45;overflow-wrap:anywhere}
