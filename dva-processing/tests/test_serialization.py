from dva_processing.serialization import parse_aov_req_json
from dva_processing.serialization import parse_ge_yaml
from dva_processing.serialization import json_to_df
...
"""
    ...
    assert parsed.contract.dataProvider == "/catalog/participants/provider-test-id"
...
def test_json_to_df():
    json = {
        "id": 1,
        "user": {"name": "john", "email": "john.smith@example.com"},
        "result": {"date": "20250101T000000Z", "score": 8.3},
    }
    mapping = {"$.user.name": "username", "$.result.score": "score"}
