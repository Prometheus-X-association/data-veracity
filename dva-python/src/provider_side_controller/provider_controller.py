...
ADMIN_URL = "http://dva-provider-side-aca-py:8031"
...
# Create connection from provider to consumer
@app.post("/connect_provider_consumer")
async def connect_provider_consumer():
    invitation_response = requests.post("http://dva-provider-side-aca-py:8031/out-of-band/create-invitation", json={
    ...
    receive_response = requests.post("http://dva-consumer-side-aca-py:8032/out-of-band/receive-invitation", json=invitation)

    if receive_response.status_code != 200:
        return {"error": "Consumer failed to receive invitation", "details": receive_response.text}

    return {"message": "Connection created between provider and consumer"}
    ...
    if target == "consumer":
        try:
            forward_resp = requests.post(
                "http://dva-consumer-side-controller:8052/receive_vc",
