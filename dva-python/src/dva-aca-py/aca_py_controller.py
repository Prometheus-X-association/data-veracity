...
ADMIN_STATIC_PAGE        = os.getenv("ADMIN_STATIC_PAGE", "index.html")     # provider index.html, consumer index_consumer.html
ADMIN_URL                = os.getenv("ADMIN_URL")                           # provider http://dva-provider-side-aca-py, consumer http://dva-consumer-side-aca-py
...
PEER_CONTROLLER_URL      = os.getenv("PEER_CONTROLLER_URL")                 # provider http://dva-consumer-side-aca-py, consumer http://dva-provider-side-aca-py
PEER_CONTROLLER_PORT_IN  = os.getenv("PEER_CONTROLLER_PORT_IN")             # consumer 8032, provider 8031
PEER_CONTROLLER_PORT_OUT = os.getenv("PEER_CONTROLLER_PORT_OUT")            # consumer 8052, provider 8051
...
async def init_all_self():
    ...
    receive_resp = requests.post(
        f"{ADMIN_URL}:{ADMIN_PORT}/out-of-band/receive-invitation",
        json=invitation
    )
    ...
    issue_resp = requests.post(
        f"{ADMIN_URL}:{ADMIN_PORT}/issue-credential-2.0/send-offer",
        json=issue_payload
    )
...
async def generate_aov(payload: AOVRequest):
    ...
    issue_resp = requests.post(f"{ADMIN_URL}:{ADMIN_PORT}/issue-credential-2.0/send", json=issue_payload)
...
async def request_aov(
    ...
    params = {"subject": subject, "issuer_id": issuer_id, "comment": comment, "target": f"{PEER_LABEL}"}
...
async def present_custom_vc():
    ...
    if not peer_conn:
        raise HTTPException(status_code=400, detail=f"No active connection found with peer '{PEER_LABEL}'.")
    ...
    return {"message": "Presentation request sent from provider to consumer.", "connection_id": connection_id}
